import { TextBkGndColorButton } from "../../../src/ToolsPanel/Inputs/TextBkGndColorButton"; 

const styleChangedMock = jest.fn().mockImplementation(() => {});

const buildButton = () => {
    const button = new TextBkGndColorButton(styleChangedMock);
...
describe('Testing click', () => {
    test('', () => {
        ...
        expect(styleChangedMock.mock.calls[0][0]).toEqual({"background-color": button.colorStripe.style.backgroundColor});
    });
});
...
describe('Testing colorChanged handler', () => {
    test('', () => {
        ...
        expect(styleChangedMock.mock.calls).toHaveLength(1);
        expect(styleChangedMock.mock.calls[0][0]).toEqual({"background-color": "rgb(30, 31, 32)"});
    });
});